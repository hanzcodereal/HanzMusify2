// ============================================================
// HANZMUSIFY - CORE PLAYER (FULL FIX)
// ============================================================
...
try{var storedAutoNext = localStorage.getItem('hanzz_auto_next');if(storedAutoNext!==null){S.autoNext = storedAutoNext==='true';}}catch(e){}
...
function updateOG(title, cover, artist) {
    if (title && cover) {
        ...
        var docTitle = fullTitle + ' | HanzMusify';
        var description = 'Dengarkan ' + fullTitle + ' di HanzMusify';
        ...
        document.title = 'HanzMusify';

        setMetaTag('og:title', 'HanzMusify', true);
        setMetaTag('og:description', 'HanzMusify - Web Music Player', true);
        setMetaTag('og:image', 'https://www.gobox.my.id/file/uZKDQkZ3c5VK.png', true);
        setMetaTag('twitter:title', 'HanzMusify', false);
        setMetaTag('twitter:description', 'HanzMusify - Web Music Player', false);
...
function toggleAutoNext(){
    ...
    try { localStorage.setItem('hanzz_auto_next', S.autoNext); } catch(e) {}
...
function getLikedSongs(){
    try{return JSON.parse(localStorage.getItem('hanzz_liked_songs')||'[]');}catch(e){return[];}
}
function saveLikedSongs(songs){
    try{localStorage.setItem('hanzz_liked_songs',JSON.stringify(songs));}catch(e){}
}
...
function getLikedArtists(){
    try{return JSON.parse(localStorage.getItem('hanzz_liked_artists')||'[]');}catch(e){return[];}
}
function saveLikedArtists(artists){
    try{localStorage.setItem('hanzz_liked_artists',JSON.stringify(artists));}catch(e){}
}
...
function getUserPlaylists(){
    try{
        var pls=JSON.parse(localStorage.getItem('hanzz_playlists')||'[]');
        var changed=false;
        ...
        if(changed){
            localStorage.setItem('hanzz_playlists',JSON.stringify(pls));
        }
        return pls;
    }catch(e){return[];}
}
...
function setPlaybackSpeed(speed) {
    ...
    try {
        localStorage.setItem('hanzz_playback_rate', speed);
    } catch(e) {}
    ...
    popup.innerHTML = '<div class="w-full max-w-sm rounded-3xl p-6 border border-white/10 glass-strong text-center" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">' +
        ...
        '<div id="share-card-preview" class="p-6 rounded-2xl mb-6 flex flex-col items-center gap-4 relative overflow-hidden" ' +
            ...
            '<div class="border-t border-white/5 w-full pt-3 mt-1 flex items-center justify-center gap-1.5">' +
                ...
                '<span class="text-[10px] text-[#6b7280] tracking-wider font-semibold uppercase">HanzMusify Web App</span>' +
            '</div>' +
        '</div>' +
